<script lang="ts">
  ...
  function toggleSelectAll(event: Event) {
    ...
    budgetData.update((current: BudgetData) => {
      current.expenses = current.expenses.map((exp) => ({
        ...exp,
        selected: checked,
      }));
      return current;
...
<div class="container">
  ...
  <p>
    Enter your incomes and planned expenses for the month. Select which expenses
    apply, then calculate your potential savings.
  </p>
    ...
    {#each $budgetData.expenses as expense, index (index)}
      <div class="expense-item">
        <input
          ...
          on:change={(e) =>
            updateExpenseSelected(index, e.currentTarget.checked)}
        />
        <label>
          Category:
          <input
            ...
            on:input={(e) =>
              updateExpenseCategory(index, e.currentTarget.value)}
          />
        </label>
        <label>
          Planned Amount:
          <input
            ...
            on:input={(e) =>
              updateExpensePlanned(index, +e.currentTarget.value)}
          />
        </label>
...
<style>
  ...
  @media (max-width: 768px) {
    /* Existing container and heading adjustments... */
    ...
    /* Stack each label’s text and input vertically */
    .expense-item label {
      flex-direction: column;
      align-items: flex-start;
      width: 100%;
    ...
    input[type="number"] {
      width: 100%;
      margin-top: 0.25rem; /* Slightly less margin so it looks tighter */
    }

    /* Adjust the delete button to appear at the end of the item */
    .delete-button {
      align-self: flex-end;
      margin-top: 0.5rem;
    }
